...
function generateSyntheticData(type: string): any[] {
    ...
    if (type.toLowerCase().includes('bar') || type.toLowerCase().includes('column')) {
...
export async function analyzeImage(file: File): Promise<VisionAnalysisResult> {
    return new Promise((resolve) => {
        setTimeout(() => {
            // Simulate detection logic (random for demo, or based on name)
            const possibleTypes = ['Bar Chart', 'Line Chart', 'Scatter Plot', 'Pie Chart', 'Heatmap'];
